...
Superseded session handoffs, newest first, moved verbatim out of [HANDOFF.md](../HANDOFF.md) when it
reached 2,800 lines and stopped being a handoff.

**This is history, not state.** Every entry was accurate when written and much of it is now wrong —
refs have moved, branches merged, "not pushed" became deployed. Do not act on anything here without
checking the code or git first. What it is good for: the REASONING behind a decision, which a commit
message often compresses away, and the shape of bugs that recurred.

Durable lessons belong in [CLAUDE.md](../CLAUDE.md) §9, not here. If something in this file still
bites, promote it there and leave the narrative behind.
