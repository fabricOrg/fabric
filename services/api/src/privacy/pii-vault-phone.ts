...
export async function subjectForPhone(input: {
  db: AppDb;
  tenantId: string;
  e164: string;
  masterKey: Buffer;
  ...
  const index = phoneBlindIndex(input.indexKey, input.tenantId, input.e164);
  const existing = (await input.db.withTenant(
    input.tenantId,
    (tx) => tx`
      SELECT subject_id FROM data_subjects
      WHERE phone_hash = ${index} AND erased_at IS NULL LIMIT 1`,
  )) as Row[];
  ...
  return input.db.withTenant(input.tenantId, async (tx) => {
    ...
    const raced = inserted[0]?.subject_id
      ...
      : (
          (await tx`
            SELECT subject_id FROM data_subjects
            WHERE phone_hash = ${index} AND erased_at IS NULL LIMIT 1`) as Row[]
        )[0]?.subject_id;
    ...
    await tx`
      ...
      SELECT current_setting('app.tenant_id')::uuid, ${id}, 'phone',
             ${encryptPii(dek, input.e164, input.tenantId, id, "phone")},
             ${String(key.dek_id)}
      WHERE NOT EXISTS (
...
export async function findSubjectForPhone(input: {
  db: AppDb;
  tenantId: string;
  e164: string;
  indexKey: Buffer;
...
export async function readPhones(input: {
  db: AppDb;
  tenantId: string;
