...
/**
 * A body the route hands straight to the browser (the statement CSV) is drained by the caller, and
 * the deadline covers the drain too — so an export budget has to fit the download, not just the
 * round trip.
 */
export const API_EXPORT_TIMEOUT_MS = 60_000;
 ...
 * gets a parseable `upstream_timeout` it can branch on — and `resolveUserSessionV2`, documented to
