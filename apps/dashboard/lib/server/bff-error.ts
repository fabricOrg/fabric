...
/**
 * Refusals the BFF raises ON ITS OWN AUTHORITY — no session, wrong role, untrusted origin, a body
 * that does not parse. A forwarded API failure is different and must keep its upstream envelope
 * verbatim: `BffError.payload` already IS one, complete with the API's `request_id`.
 *
...
/** 403 — authenticated, but the membership role does not clear this route's gate. */
