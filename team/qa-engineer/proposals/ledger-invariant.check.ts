 ...
 */
export async function findImbalancedTxns(db: SqlExecutor): Promise<TxnImbalance[]> {
  ...
  `);
  return rows.map((r) => ({ txnId: String(r.txn_id), netMinor: String(r.net_minor) }));
}
 ...
 */
export async function findDriftedAccounts(db: SqlExecutor): Promise<AccountDrift[]> {
  const { rows } = await db.query(`
 ...
 */
export async function checkLedgerInvariants(db: SqlExecutor): Promise<LedgerInvariantResult> {
...
export function formatViolations(r: LedgerInvariantResult): string {
  if (r.ok) return "ledger invariants OK (trial balance + projection integrity)";
  const lines: string[] = [];
  if (r.imbalancedTxns.length) {
    lines.push(`✗ ${r.imbalancedTxns.length} txn(s) do not balance (Σ signed legs ≠ 0):`);
    for (const t of r.imbalancedTxns) lines.push(`    txn ${t.txnId}: net ${t.netMinor} minor`);
  }
  if (r.driftedAccounts.length) {
    lines.push(`✗ ${r.driftedAccounts.length} account(s) drifted (balance_minor ≠ Σ legs):`);
    for (const a of r.driftedAccounts)
      lines.push(`    ${a.kind}/${a.currency} ${a.accountId}: stored ${a.storedBalanceMinor} ≠ derived ${a.derivedBalanceMinor}`);
  }
  return lines.join("\n");
}
